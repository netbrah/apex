...
describe('handleAtCommand', () => {
  ...
  beforeEach(async () => {
    ...
    mockConfig = {
      ...
      getFileFilteringRespectApexIgnore: () => true,
  ...
  describe('qwen-ignore filtering', () => {
    it('should skip qwen-ignored files in @ commands', async () => {
      ...
      const apexIgnoredFile = await createTestFile(
        path.join(testRootDir, 'build', 'output.js'),
        'console.log("Hello");',
      );
      const query = `@${apexIgnoredFile}`;
  ...
  it('should process non-ignored files when .apexignore is present', async () => {
    await createTestFile(
      path.join(testRootDir, '.apexignore'),
      'build/output.js',
    );
  ...
  it('should handle mixed qwen-ignored and valid files', async () => {
    ...
    const query = `@${validFile} @${apexIgnoredFile}`;
    ...
    expect(result).toMatchObject({
      processedQuery: [
        { text: `@${validFile} @${apexIgnoredFile}` },
        { text: '\n--- Content from referenced files ---' },
        { text: `\nContent from ${validFile}:\n` },
