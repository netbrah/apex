...
import * as coreFunctions from '@apex-code/apex-core';
...
describe('setupWorktree', () => {
  ...
  beforeEach(() => {
    ...
    mockService.setup.mockResolvedValue({
      name: 'my-feature',
      path: '/mock/project/.apex/worktrees/my-feature',
      baseSha: 'base-sha',
    });
  });
  ...
  it('should create and switch to a new worktree', async () => {
    ...
    expect(process.chdir).toHaveBeenCalledWith(
      '/mock/project/.apex/worktrees/my-feature',
    );
    expect(process.env['APEX_WORKTREE_HANDLED']).toBe('1');
  });

  it('should generate a name if worktreeName is undefined', async () => {
    mockService.setup.mockResolvedValue({
      name: 'generated-name',
      path: '/mock/project/.apex/worktrees/generated-name',
      baseSha: 'base-sha',
    });
  ...
  it('should skip worktree creation if APEX_WORKTREE_HANDLED is set', async () => {
    process.env['APEX_WORKTREE_HANDLED'] = '1';
