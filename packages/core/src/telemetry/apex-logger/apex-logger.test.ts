...
import { ApexLogger, TEST_ONLY } from './apex-logger.js';
...
describe('ApexLogger', () => {
  ...
  describe('logHookCallEvent', () => {
    ...
    it('should log a failed hook call event with error when telemetry log prompts enabled', () => {
      ...
      const logger = ApexLogger.getInstance(configWithLogPrompts)!;
      const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
    ...
    it('should not include error when telemetry log prompts disabled', () => {
      ...
      (ApexLogger as unknown as { instance: undefined }).instance = undefined;
      const logger = ApexLogger.getInstance(configWithoutLogPrompts)!;
      const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
    ...
    it('should sanitize hook name to remove sensitive information', () => {
      ...
      const event = new HookCallEvent(
        'PreToolUse',
        'command',
        '/home/user/.apex/hooks/check-secrets.sh --api-key=secret123',
        { tool_name: 'read_file' },
        100,
        true,
      );
