...
describe('ClearcutLogger', () => {
  ...
  describe('createLogEvent', () => {
    ...
    it('logs the GPU information (single GPU)', async () => {
      ...
      const gpuInfoEntry = event?.event_metadata[0].find(
        (item) => item.gemini_cli_key === EventMetadataKey.APEX_GPU_INFO,
      );
    ...
    it('logs multiple GPUs', async () => {
      ...
      const gpuInfoEntry = metadata?.find(
        (m) => m.gemini_cli_key === EventMetadataKey.APEX_GPU_INFO,
      );
    ...
    it('handles empty os.cpus() gracefully', async () => {
      ...
      const cpuInfoEntry = metadata?.find(
        (m) => m.gemini_cli_key === EventMetadataKey.APEX_CPU_INFO,
      );
      expect(cpuInfoEntry).toBeUndefined();

      const cpuCoresEntry = metadata?.find(
        (m) => m.gemini_cli_key === EventMetadataKey.APEX_CPU_CORES,
      );
  ...
  describe('GH_WORKFLOW_NAME metadata', () => {
    ...
    it('does not include workflow name when GH_WORKFLOW_NAME is not set', () => {
      ...
      const hasWorkflowName = event?.event_metadata[0].some(
        (item) =>
          item.gemini_cli_key === EventMetadataKey.APEX_GH_WORKFLOW_NAME,
      );
  ...
  describe('GITHUB_REPOSITORY metadata', () => {
    it('includes hashed repository when GITHUB_REPOSITORY is set', () => {
      vi.stubEnv('GITHUB_REPOSITORY', 'google/apex');
      const { logger } = setup({});
      ...
      expect(repositoryMetadata?.value).not.toBe('google/apex');
    });
  ...
  describe('logChatCompressionEvent', () => {
    it('logs an event with proper fields', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_COMPRESSION_TOKENS_BEFORE,
        '9001',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_COMPRESSION_TOKENS_AFTER,
        '8000',
      ]);
    });
  });
  ...
  describe('enqueueLogEvent', () => {
    ...
    it('should evict the oldest event when the queue is full', () => {
      ...
      for (let i = 0; i < TEST_ONLY.MAX_EVENTS; i++) {
        logger!.enqueueLogEvent(
          logger!.createLogEvent(EventNames.API_ERROR, [
            {
              gemini_cli_key: EventMetadataKey.APEX_AI_ADDED_LINES,
              value: `${i}`,
            },
          ]),
        );
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_AI_ADDED_LINES,
        '0',
      ]);
  ...
  describe('logModelRoutingEvent', () => {
    it('logs a successful routing event', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_ROUTING_DECISION,
        'gemini-pro',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_ROUTING_DECISION_SOURCE,
        'default-strategy',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_ROUTING_LATENCY_MS,
        '123',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_ROUTING_FAILURE,
        'false',
      ]);
    });
    ...
    it('logs a failed routing event with a reason', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_ROUTING_FAILURE_REASON,
        'Something went wrong',
      ]);
    });
    ...
    it('logs a successful routing event with numerical routing fields', () => {
      ...
      expect(events[0]).toHaveEventName(EventNames.MODEL_ROUTING);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_ROUTING_REASONING,
        '[Score: 90 / Threshold: 80] reasoning',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_ROUTING_NUMERICAL_ENABLED,
        'true',
      ]);
      expect(events[0]).toHaveMetadataValue([
  ...
  describe('logAgentStartEvent', () => {
    it('logs an event with proper fields', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_AGENT_ID,
        'agent-123',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_AGENT_NAME,
        'TestAgent',
      ]);
    });
  });
  ...
  describe('logExperiments', () => {
    it('async path includes exp.gws_experiment field with experiment IDs', async () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_EXPERIMENT_IDS,
        '123,456,789',
      ]);
  ...
  describe('logAgentFinishEvent', () => {
    it('logs an event with proper fields (success)', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_AGENT_DURATION_MS,
        '1000',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_AGENT_TURN_COUNT,
        '5',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_AGENT_TERMINATE_REASON,
        'GOAL',
      ]);
    });
  ...
  describe('logToolCallEvent', () => {
    it('logs an event with all diff metadata', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_AI_REMOVED_LINES,
        '2',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_AI_ADDED_CHARS,
        '3',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_AI_REMOVED_CHARS,
        '4',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_USER_ADDED_LINES,
        '5',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_USER_REMOVED_LINES,
        '6',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_USER_ADDED_CHARS,
        '7',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_USER_REMOVED_CHARS,
        '8',
      ]);
    });
    ...
    it('logs AskUser tool metadata', () => {
      ...
      expect(events[0]).toHaveEventName(EventNames.TOOL_CALL);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_ASK_USER_QUESTION_TYPES,
        JSON.stringify(['choice', 'text']),
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_ASK_USER_DISMISSED,
        'false',
      ]);
      expect(events[0]).toHaveMetadataValue([
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_ASK_USER_ANSWER_COUNT,
        '2',
      ]);
    });
  ...
  describe('logWebFetchFallbackAttemptEvent', () => {
    it('logs an event with the proper name and reason', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_WEB_FETCH_FALLBACK_REASON,
        'private_ip',
      ]);
    });
  });
  ...
  describe('logHookCallEvent', () => {
    it('logs an event with proper fields', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_HOOK_EVENT_NAME,
        'before-tool',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_HOOK_DURATION_MS,
        '150',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_HOOK_SUCCESS,
        'true',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_HOOK_EXIT_CODE,
        '0',
      ]);
    });
  });
  ...
  describe('logCreditsUsedEvent', () => {
    it('logs an event with model, consumed, and remaining credits', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_BILLING_MODEL,
        '"gemini-3-pro-preview"',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_BILLING_CREDITS_CONSUMED,
        '10',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_BILLING_CREDITS_REMAINING,
        '490',
      ]);
    });
  });
  ...
  describe('logOverageOptionSelectedEvent', () => {
    it('logs an event with model, selected option, and credit balance', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_BILLING_SELECTED_OPTION,
        '"use_credits"',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_BILLING_CREDIT_BALANCE,
        '350',
      ]);
    });
  });
  ...
  describe('logCreditPurchaseClickEvent', () => {
    it('logs an event with model and source', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_BILLING_PURCHASE_SOURCE,
        '"empty_wallet_menu"',
      ]);
    });
  });
  ...
  describe('logOnboardingStartEvent', () => {
    it('logs an event with proper name and start key', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_ONBOARDING_START,
        'true',
      ]);
    });
  });
  ...
  describe('logOnboardingSuccessEvent', () => {
    it('logs an event with proper name and user tier', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_ONBOARDING_USER_TIER,
        'standard-tier',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.APEX_ONBOARDING_DURATION_MS,
        '100',
      ]);
    });
  });
