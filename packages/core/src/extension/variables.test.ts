...
describe('substituteHookVariables', () => {
  ...
  it('should handle multiple event types with hooks', () => {
    const basePath = '/home/user/.apex/extensions/my-extension';
    ...
    expect(result!['PreToolUse']).toHaveLength(1);
    expect(result!['PreToolUse']![0].hooks![0].command).toBe(
      '/home/user/.apex/extensions/my-extension/scripts/pre-start.sh',
    );
    expect(result!['UserPromptSubmit']).toHaveLength(1);
    expect(result!['UserPromptSubmit']![0].hooks![0].command).toBe(
      '/home/user/.apex/extensions/my-extension/setup/install.py',
    );
  });
...
describe('performVariableReplacement', () => {
  ...
  it('should replace .claude with .apex in markdown files', () => {
    const extDir = path.join(testDir, 'ext');
    fs.mkdirSync(extDir, { recursive: true });
    ...
    expect(result).toContain('.apex/loop.local.md');
    expect(result).toContain('rm .apex/loop.local.md');
    expect(result).toContain('$HOME/.apex/cache');
    expect(result).toContain('./.apex/local');
  ...
  it('should replace .claude with .apex in shell scripts', () => {
    const extDir = path.join(testDir, 'ext');
    fs.mkdirSync(extDir, { recursive: true });
    ...
    expect(result).toContain('~/.apex/cache');
    expect(result).toContain('./.apex/local');
    expect(result).toContain('.apex/config');
    // Note: URLs are also being replaced in current implementation
    expect(result).toContain('https://example.com/.apex/page');
  });
