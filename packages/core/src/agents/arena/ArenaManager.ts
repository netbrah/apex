...
export class ArenaManager {
  ...
  constructor(config: Config, callbacks: ArenaCallbacks = {}) {
    ...
    // Use the user-configured base dir, or default to ~/.apex/arena.
    this.arenaBaseDir =
  ...
  private buildAgentSpawnConfig(agent: ArenaAgentState): AgentSpawnConfig {
    ...
    if (model.apiKey) {
      env['APEX_API_KEY'] = model.apiKey;
    }
    if (model.baseUrl) {
      env['APEX_BASE_URL'] = model.baseUrl;
    }
   ...
   * `~/.apex/worktrees/<sessionId>/`.  The directory contains:
