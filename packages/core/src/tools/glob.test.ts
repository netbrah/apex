...
describe('GlobTool', () => {
  ...
  describe('ignore file handling', () => {
    ...
    it('should not respect .apexignore when respect_gemini_ignore is false', async () => {
      await fs.writeFile(
        path.join(tempRootDir, GEMINI_IGNORE_FILE_NAME),
        'gemini-ignored_test.txt',
      );
  ...
  describe('ignore file handling', () => {
    ...
    it('should respect .apexignore when searching a subdirectory (path option)', async () => {
      await fs.writeFile(path.join(tempRootDir, '.apexignore'), '*.secret');
      await fs.writeFile(path.join(tempRootDir, 'sub', 'visible.txt'), 'ok');
      await fs.writeFile(
        path.join(tempRootDir, 'sub', 'hidden.secret'),
        'should be ignored',
      ...
      // Recreate to pick up .apexignore
      const subDirTool = new GlobTool(mockConfig);
      const params: GlobToolParams = { pattern: '*', path: 'sub' };
      const invocation = subDirTool.build(params);
      const result = await invocation.execute(abortSignal);
