...
enum class KmStatus { OK = 0, NOT_FOUND = 1, INTERNAL_ERROR = 2 };
...
/// Validate a raw key blob.
bool validateKeyBlob(const std::string& blob, uint32_t expected_type) {
    if (blob.empty() || blob.size() < 32) return false;
    return static_cast<uint8_t>(blob[0]) == (expected_type & 0xFF);
}

/// Derive a key ID from svm_uuid.
std::string deriveKeyId(const std::string& svm_uuid, uint32_t key_type) {
    std::ostringstream oss;
    oss << svm_uuid << ":" << key_type;
    return oss.str();
}

static KmStatus pushKeyToKmipServer(const KeyEntry& entry, const std::string& url) {
    if (url.empty()) return KmStatus::INTERNAL_ERROR;
    if (!validateKeyBlob(entry.key_data, entry.key_type)) return KmStatus::INTERNAL_ERROR;
    return KmStatus::OK;
}
...
def make_fixture(tmpdir: str) -> tuple[str, str]:
    ...
    cc = os.path.join(tmpdir, "km_smoke.cc")
    db = os.path.join(tmpdir, "compile_commands.json")

    with open(cc, "w") as f:
        f.write(FIXTURE_CC)

    # Detect compiler args: try xcrun on Mac, fallback to plain clang++
    extra_args: list[str] = []
    try:
        sdk = subprocess.run(
            ["xcrun", "--show-sdk-path"],
    ...
    entry = {
        "directory": tmpdir,
        "file": cc,
        "arguments": ["clang++", "-std=c++17", *extra_args, "-c", cc, "-o", cc+".o"],
    }
    with open(db, "w") as f:
        json.dump([entry], f)
...
def run_checks(m, cc: str, db: str, verbose: bool) -> tuple[int, int]:
    ...
    try:
        ...
        for i, l in enumerate(lines):
            if "validateKeyBlob" in l and "bool " not in l:
                call_line = i + 1
    ...
    try:
        ...
        def_line = next(
            (i+1 for i, l in enumerate(lines) if "bool validateKeyBlob" in l), None
        )
