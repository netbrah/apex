...
This document explains how to run APEX inside a sandbox to reduce risk when tools execute shell commands or modify files.

## Prerequisites

Before using sandboxing, you need to install and set up APEX:
...
> [!note]
>
> **Naming note:** Some sandbox-related environment variables may have used the `GEMINI_*` prefix historically. All new environment variables use the `APEX_*` prefix.
...
By default, APEX uses a published sandbox image (configured in the CLI package) and will pull it as needed.
...
export APEX_SANDBOX=true   # true auto-picks a provider (see notes below)
...
>
> - On **macOS**, `APEX_SANDBOX=true` typically selects `sandbox-exec` (Seatbelt) if available.
> - On **Linux/Windows**, `APEX_SANDBOX=true` requires `docker` or `podman` to be installed.
> - To force a provider, set `APEX_SANDBOX=docker|podman|sandbox-exec`.

## Configuration

### Enable sandboxing (in order of precedence)

1. **Environment variable**: `APEX_SANDBOX=true|false|docker|podman|sandbox-exec`
...
> [!important]
>
> If `APEX_SANDBOX` is set, it **overrides** the CLI flag and `settings.json`.

### Configure the sandbox image (Docker/Podman)

- **CLI flag**: `--sandbox-image <image>`
- **Environment variable**: `APEX_SANDBOX_IMAGE=<image>`

If you don’t set either, APEX uses the default image configured in the CLI package (for example `ghcr.io/netbrah/apex:<version>`).
...
1. Create a file named `.apex/sandbox-macos-<profile_name>.sb` in your project.
...
- Set `APEX_SANDBOX_PROXY_COMMAND=<command>`
...
On Linux, APEX defaults to enabling UID/GID mapping so the sandbox runs as your user (and reuses the mounted `~/.qwen`). Override with:
...
- Container sandbox: add them via `.apex/sandbox.Dockerfile` or `.apex/sandbox.bashrc`.
...
The official APEX Docker image is intentionally minimal to keep the image small, secure, and fast to pull. Different users require different language runtimes (Java, Python, Node.js, etc.), and bundling all environments into a single image is not practical. Therefore, Java is **not included by default** in the Docker sandbox.

If your workflow requires Java, you can extend the base image by creating a `.apex/sandbox.Dockerfile` in your project:

```dockerfile
FROM ghcr.io/netbrah/apex:latest
...
**Note:** If you have `DEBUG=true` in a project's `.env` file, it won't affect the CLI due to automatic exclusion. Use `.apex/.env` files for APEX-specific debug settings.
