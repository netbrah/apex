...
describe('SkillManager', () => {
  ...
  const validSkillConfig: SkillConfig = {
    ...
    description: 'A test skill',
    level: 'project',
    filePath: '/test/project/.apex/skills/test-skill/SKILL.md',
...
`;
    ...
    it('should determine level from file path', () => {
      const projectPath = '/test/project/.apex/skills/test-skill/SKILL.md';
      const userPath = '/home/user/.apex/skills/test-skill/SKILL.md';
...
`;
  ...
  describe('listSkills', () => {
    beforeEach(() => {
      ...
      const userQwenSkillsDir = path.join('/home/user', '.apex', 'skills');
...
Skill 3 content`);
    ...
    it('should deduplicate same-name skills across provider dirs within a level', async () => {
      ...
      const projectQwenDir = path.join('/test/project', '.apex', 'skills');
      ...
      vi.mocked(fs.readFile).mockImplementation((filePath) => {
        const pathStr = String(filePath);
        if (pathStr.includes('.apex') && pathStr.includes('shared-skill')) {
          return Promise.resolve(
  ...
  describe('getSkillsBaseDirs', () => {
    it('should return all project-level base dirs', () => {
      ...
      expect(baseDirs).toContain(path.join('/test/project', '.apex', 'skills'));
    ...
    it('should return all user-level base dirs', () => {
      ...
      expect(baseDirs).toHaveLength(2);
      expect(baseDirs).toContain(path.join('/home/user', '.apex', 'skills'));
  ...
  describe('bundled skills', () => {
    ...
    const projectDirSegment = path.join('.apex', 'skills');
    const userDirSegment = path.join('.apex', 'skills');
    ...
    function mockReaddirForLevels(levels: Set<string>) {
      vi.mocked(fs.readdir).mockImplementation((dirPath) => {
        const pathStr = String(dirPath);
        const isBundled =
          pathStr.endsWith(bundledDirSegment) && !pathStr.includes('.apex');
        const isProject =
