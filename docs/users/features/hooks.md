# APEX Hooks Documentation

## Overview

APEX hooks provide a powerful mechanism for extending and customizing the behavior of the APEX application. Hooks allow users to execute custom scripts or programs at specific points in the application lifecycle, such as before tool execution, after tool execution, at session start/end, and during other key events.

> **⚠️ EXPERIMENTAL FEATURE**
>
> Hooks are currently in an experimental stage. To enable hooks, start APEX with the `--experimental-hooks` flag:
>
> ```bash
> apex --experimental-hooks
> ```

## What are Hooks?

Hooks are user-defined scripts or programs that are automatically executed by APEX at predefined points in the application flow. They allow users to:
...
The APEX hook system consists of several key components:
...
The following table lists all available hook events in APEX:
...
{
  "decision": "block",
  "reason": "Must be provided when APEX is blocked from stopping"
}
```
...
Hooks are configured in APEX settings, typically in `.apex/settings.json` or user configuration files:
...
Configure in `.apex/settings.json`:
