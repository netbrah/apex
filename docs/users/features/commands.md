...
This document details all commands supported by APEX, helping you efficiently manage sessions, customize the interface, and control its behavior.

APEX commands are triggered through specific prefixes and fall into three categories:

| Prefix Type                | Function Description                                | Typical Use Case                                                 |
| -------------------------- | --------------------------------------------------- | ---------------------------------------------------------------- |
| Slash Commands (`/`)       | Meta-level control of APEX itself                   | Managing sessions, modifying settings, getting help              |
...
Slash commands are used to manage APEX sessions, interface, and basic behavior.
...
| `/theme`     | Change APEX visual theme                 | `/theme`                      |
...
| `/bug`      | Submit issue about APEX                         | `/bug Button click unresponsive` |
| `/copy`     | Copy last output content to clipboard           | `/copy`                          |
| `/quit`     | Exit APEX immediately                           | `/quit` or `/exit`               |
...
In addition to the in-session `/auth` slash command, APEX provides standalone CLI subcommands for managing authentication directly from the terminal:

| Command                                              | Description                                       |
| ---------------------------------------------------- | ------------------------------------------------- |
| `qwen auth`                                          | Interactive authentication setup                  |
| `qwen auth openai`                                   | Authenticate with OpenAI-compatible API           |
...
> [!tip]
>
> These commands run outside of a APEX session. Use them to configure authentication before starting a session, or in scripts and CI environments. See the [Authentication](../configuration/auth) page for full details.
...
Exclamation commands allow you to execute system commands directly within APEX.
...
Environment Variables: Commands executed via `!` will set the `APEX_CODE=1` environment variable.
...
| Global Commands  | `~/.apex/commands/`                        | Available in all projects              | Low      | Personal frequently used commands, cross-project use |
| Project Commands | `<project root directory>/.apex/commands/` | Project-specific, version-controllable | High     | Team sharing, project-specific commands              |
...
| ---------------------------------------- | ----------------- | --------------------- |
| `~/.apex/commands/test.md`               | `/test`           | `/test Parameter`     |
| `<project>/.apex/commands/git/commit.md` | `/git:commit`     | `/git:commit Message` |
...
| ----------------------------- | ----------------------------------------- |
| 1. Create directory structure | `mkdir -p ~/.apex/commands/refactor`      |
| 2. Create command file        | `touch ~/.apex/commands/refactor/pure.md` |
