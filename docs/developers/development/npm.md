...
This monorepo contains two main packages: `@apex/apex` and `@apex/apex-core`.

## `@apex/apex`

This is the main package for APEX. It is responsible for the user interface, command parsing, and all other user-facing functionality.

When this package is published, it is bundled into a single executable file. This bundle includes all of the package's dependencies, including `@apex/apex-core`. This means that whether a user installs the package with `npm install -g @apex/apex` or runs it directly with `npx @apex/apex`, they are using this single, self-contained executable.
...
Releases are managed through the [release.yml](https://github.com/netbrah/apex/actions/workflows/release.yml) GitHub Actions workflow. To perform a manual release for a patch or hotfix:
...
# Preview
npm install -g @apex/apex@preview

# Nightly
npm install -g @apex/apex@nightly
```
...
- `npx -y @apex/apex@latest --version` to validate the push worked as expected if you were not doing a rc or dev tag
- `npx -y @apex/apex@<release tag> --version` to validate the tag pushed appropriately
- _This is destructive locally_ `npm uninstall @apex/apex && npm uninstall -g @apex/apex && npm cache clean --force &&  npm install @apex/apex@<version>`
...
1.  Go to the [Actions tab](https://github.com/netbrah/apex/actions/workflows/release.yml) of the repository.
...
You can then inspect the generated tarballs to ensure that they contain the correct files and that the `package.json` files have been updated correctly. The tarballs will be created in the root of each package's directory (e.g., `packages/cli/apex-0.1.6.tgz`).
...
- **Simplified Script Execution**: You can run scripts in any package from the root of the project using the `--workspace` flag. For example, to run the `build` script in the `cli` package, you can run `npm run build --workspace @apex/apex`.
