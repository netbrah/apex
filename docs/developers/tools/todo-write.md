...
This document describes the `todo_write` tool for APEX.
...
## How to use `todo_write` with APEX
...
The tool stores todo lists in your home directory (`~/.apex/todos/`) with session-specific files, so each coding session maintains its own task list.
