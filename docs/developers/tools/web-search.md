...
1. **DashScope** (Official, Free) - Automatically available for OpenAI-compatible API users (200 requests/minute, 1000 requests/day)
...
- **OpenAI-compatible API users:** DashScope is automatically added to your provider list, even if not explicitly configured
...
Pass API keys when running APEX:

```bash
# Tavily
apex --tavily-api-key tvly-xxxxx

# Google
apex --google-api-key your-key --google-search-engine-id your-id

# Specify default provider
apex --web-search-default tavily
```
...
**Note:** This setting requires a restart of APEX to take effect. Once disabled, the `web_search` tool will not be available to the model, even if web search providers are configured.
...
- **Cost:** Free
- **Authentication:** Automatically available when using OpenAI-compatible API authentication
- **Configuration:** No API key required, automatically added to provider list for OpenAI-compatible API users
- **Quota:** 200 requests/minute, 1000 requests/day
- **Best for:** General queries, always available as fallback for OpenAI-compatible API users
- **Auto-registration:** If you're using OpenAI-compatible API, DashScope is automatically added to your provider list even if you don't configure it explicitly
...
- **DashScope availability:** Automatically available for OpenAI-compatible API users, no configuration needed
...
- **For OpenAI-compatible API users:** The tool is automatically registered with DashScope provider, no configuration needed
...
- Check your configuration: Run `apex` and use the settings dialog
- View your current settings in `~/.apex/settings.json` (macOS/Linux) or `%USERPROFILE%\.apex\settings.json` (Windows)
