...
This document describes the `web_fetch` tool for APEX.
...
## How to use `web_fetch` with APEX

To use `web_fetch` with APEX, provide a URL and a prompt describing what you want to extract from that URL. The tool will ask for confirmation before fetching the URL. Once confirmed, the tool will fetch the content directly and process it using an AI model.
...
```
web_fetch(url="https://github.com/netbrah/Qwen/blob/main/README.md", prompt="What are the installation steps and main features?")
```
