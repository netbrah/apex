...
Your choice is saved in a central file (`~/.apex/trustedFolders.json`), so you
...
1.  **Workspace settings are ignored**: The CLI will **not** load the
    `.apex/settings.json` file from the project. This prevents the loading of
...
- **View all trust rules**: To see a complete list of all your trusted and
  ...
  `~/.apex/trustedFolders.json` file in your home directory.
...
2.  **Local trust file**: If the IDE is not connected, the CLI checks the
    central `~/.apex/trustedFolders.json` file.
