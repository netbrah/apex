...
1.  Run the following command to create the folders:
    ...
    **macOS/Linux**
    ...
    ```bash
    mkdir -p .apex/skills/api-auditor/scripts
    ```
...
1.  Create a file at `.apex/skills/api-auditor/SKILL.md`. This tells the agent
...
1.  Create a file at `.apex/skills/api-auditor/scripts/audit.js`. This is the
    code the agent will run.

    ```javascript
    // .apex/skills/api-auditor/scripts/audit.js
...
Gemini CLI automatically discovers skills in the `.apex/skills` directory. You
...
`node .apex/skills/api-auditor/scripts/audit.js http://geminili.com`
