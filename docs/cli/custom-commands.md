...
1.  **User commands (global):** Located in `~/.apex/commands/`. These commands
    are available in any project you are working on.
2.  **Project commands (local):** Located in
    `<your-project-root>/.apex/commands/`. These commands are specific to the
...
- A file at `~/.apex/commands/test.toml` becomes the command `/test`.
- A file at `<project>/.apex/commands/git/commit.toml` becomes the namespaced
  command `/git:commit`.
...
```toml
# In: <project>/.apex/commands/changelog.toml
...
````toml
# In: <project>/.apex/commands/git/commit.toml
...
```toml
# In: <project>/.apex/commands/review.toml
...
```bash
mkdir -p ~/.apex/commands/refactor
touch ~/.apex/commands/refactor/pure.toml
```
...
Open `~/.apex/commands/refactor/pure.toml` in your editor and add the
following content. We are including the optional `description` for best
practice.

```toml
# In: ~/.apex/commands/refactor/pure.toml
