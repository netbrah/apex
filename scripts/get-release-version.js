...
function getVersionFromNPM(distTag) {
  const command = `npm view @apex-code/apex version --tag=${distTag}`;
  try {
...
function getAllVersionsFromNPM() {
  const command = `npm view @apex-code/apex versions --json`;
  try {
...
function isVersionDeprecated(version) {
  const command = `npm view @apex-code/apex@${version} deprecated`;
  try {
...
function doesVersionExist(version) {
  // Check NPM
  try {
    const command = `npm view @apex-code/apex@${version} version 2>/dev/null`;
    const output = execSync(command).toString().trim();
    if (output === version) {
